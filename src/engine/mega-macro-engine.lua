...
local function getTexture(macro, macroIndex)
	-- Safety check: if macro is nil, we can't get texture from it
	if not macro then return MegaMacroTexture end
...
local function MergeCharacterSpecializationMacros()
	-- FIX: Safety checks to prevent "index field '?' (a nil value)"
	if not MegaMacroCachedSpecialization or MegaMacroCachedSpecialization == "" then return end
	if not MegaMacroCharacterData or not MegaMacroCharacterData.Specializations then return end
	
	local specData = MegaMacroCharacterData.Specializations[MegaMacroCachedSpecialization]
	if not specData then return end -- Specialization not initialized in DB yet

	local characterSpecializationMacros = specData.Macros
...
local function BindMacros()
	...
	-- FIX: Safety check for Specialization entry
	if MegaMacroCachedSpecialization and MegaMacroCharacterData.Specializations and MegaMacroCharacterData.Specializations[MegaMacroCachedSpecialization] then
...
local function PickupMacroWrapper(original, macroIndex)
	...
	if MegaMacroConfig and MegaMacroConfig['UseNativeActionBar'] then
		if original then original(macroIndex) end
		return
	end
...
function MegaMacroEngine.SafeInitialize()
	...
	-- [[ FIX: Stack Overflow Prevention ]]
	-- Only hook C_Macro.PickupMacro if it exists. 
	-- If it does not exist (legacy), hook _G.PickupMacro. 
	-- NEVER hook both.
	
	if C_Macro and C_Macro.PickupMacro then
		local originalCPickup = C_Macro.PickupMacro
		C_Macro.PickupMacro = function(macroIndex) 
			PickupMacroWrapper(originalCPickup, macroIndex) 
		end
	elseif _G.PickupMacro then
		local originalPickup = _G.PickupMacro
		_G.PickupMacro = function(macroIndex) 
			PickupMacroWrapper(originalPickup, macroIndex) 
		end
	end
...
function MegaMacroEngine.ImportMacros()
	...
	elseif errorMessage then
		print(errorMessage) -- Replaced message() with print()
	end
...
function MegaMacroEngine.Uninstall()
	...
	for i=1, MacroLimits.MaxGlobalMacros + MacroLimits.MaxCharacterMacros do
		...
		if macroId then
			...
			if macro then
				if macro.Code and #macro.Code > MegaMacroCodeMaxLengthForNative then
					cleanCode = macro.Code
				end
				-- 12.0 Fix: Safe getTexture call
				iconTexture = getTexture(macro, i)
			else
				-- 12.0 Fix: Orphan macro (ID exists but data missing), preserve current icon
				local _, currentIcon = GetMacroInfo(i)
				iconTexture = currentIcon
			end
	...
	-- 12.0 Fix: message() is deprecated, use print() instead
	print("Mega Macro: Uninstalled. Disabled MegaMacro and reload your UI.")
	return true
end