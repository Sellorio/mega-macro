...
function MegaMacroConfig_GetWindowPosition()
    if MegaMacroGlobalData.WindowInfo then
        return MegaMacroGlobalData.WindowInfo.RelativePoint, MegaMacroGlobalData.WindowInfo.X,
            MegaMacroGlobalData.WindowInfo.Y
    end
end
...
function MecaMacro_GenerateConfig()
    ...
    MecaMacroConfig_GenerateCheckbox('UseBlizzardMacros', 'Blizzard Style Macros',
        '250 Char limit. \nUses Blizzards Macro Buttons while keeping MegaMacro interface and organizer. \n\nEnable this if you have stability issues or need to uninstall MegaMacro. \n\nNote: Uninstalling may still cause you to lose class and spec Macros since they are not supported by Blizzard. Move them to global or character tabs before uninstalling.'
        , 0, 0, MegaMacroConfig.UseNativeMacros, function(value)
        MegaMacroConfig.UseNativeMacros = value
        MegaMacro_BlizMacro_Toggle()
    end)
    -- Blizzard Action Bar Icons
    MecaMacroConfig_GenerateCheckbox('UseBlizzardActionIcons', 'Blizzard Action Bar Icons',
        'Disable Mega Macro Action Bar Engine. \nOnly Use with Blizzard Style Macros', 0, -25,
        MegaMacroConfig.UseNativeActionBar, function(value)
        MegaMacroConfig.UseNativeActionBar = value
    end)
end

function MecaMacroConfig_GenerateCheckbox(name, text, tooltip, x, y, checked, onClick)
    local checkbox = CreateFrame("CheckButton", "MegaMacro_ConfigContainer_" .. name, MegaMacro_ConfigContainer,
