...
local function GetMacroButtonUI(index)
    ...
    if btn then
        return btn, _G[buttonName].Name, _G[buttonName].Icon
    end
end
...
local function CreateMacroSlotFrames()
    for i=1, HighestMaxMacroCount do
        -- 12.0: If using CheckButton, BackdropTemplate isn't strictly required unless defining Backdrops, 
        -- but good practice if you style them later.
...
local function InitializeIconListPanel()
    if not IconListInitialized then
        -- Ensure BuildIconArray is defined (usually in Blizzard's UI code or your own utils). 
        -- If it's a custom global, ensure it exists.
        if BuildIconArray then
...
local function InitializeTabs()
    ...
    if MegaMacro_FrameTab2 then MegaMacro_FrameTab2:SetText(MegaMacroCachedClass or "Class") end
    if MegaMacro_FrameTab4 then MegaMacro_FrameTab4:SetText(playerName) end
    if MegaMacro_FrameTab5 then MegaMacro_FrameTab5:SetText("Inactive") end
    if MegaMacro_FrameTab6 then MegaMacro_FrameTab6:SetText("Config") end

    if MegaMacro_FrameTab3 then
        if not MegaMacroCachedSpecialization or MegaMacroCachedSpecialization == '' then
...
local function InitializeMacroSlots()
    ...
    for i=1, scopeSlotCount do
        ...
        if buttonFrame == nil then break end
        buttonFrame:Show()
    end
...
local function RefreshSelectedMacroIcon()
    ...
    if SelectedMacro then
        if SelectedIcon == MegaMacroTexture then
            ...
            -- 12.0: select(4, ...) gets the icon from the return values
    ...
    for i=1, HighestMaxMacroCount do
        ...
        if buttonFrame then
            ...
            if macro then
                ...
                -- Check if actually synced to native
...
local function UpdateTooltipIfButtonIsHovered(updatedMacroId)
    -- 12.0: GetMouseFoci is the standard, GetMouseFocus is deprecated
...
local function HandleReceiveDrag(targetScope)
    ...
    if type == "macro" then
        ...
        if macroId then
            ...
            ClearCursor()
            -- 12.0 C_Macro usage for importing
            local name, _, body = C_Macro.GetMacroInfo(macroIndex)
            ...
            if not InCombatLockdown() then
                ...
                if newMacroIndex then
                    for i=1, 120 do
                        ...
                        if actionButtonType == "macro" and actionButtonArg == macroIndex then
                            C_Macro.PickupMacro(newMacroIndex)
                            PlaceAction(i)
                            ClearCursor()
                        end
                    end
                ...
                C_Macro.DeleteMacro(macroIndex)
            end
        end
    end
    ...
    Show = function()
        if MegaMacroConfig_IsWindowDialog() then
            ...
            if relativePoint then
                ...
                MegaMacro_Frame:SetPoint("CENTER")
            end
...
function MegaMacro_RegisterShiftClicks()
    -- 12.0 Shift-Click handler (Modern)
    local function shiftClickHookFunction(self) 
        -- With 12.0, SpellBook slots are complex. We check if the button has a specific ID or SpellID attached.
        -- This logic depends on the new SpellBook UI structure.
        -- For safety, we only insert if ChatEdit is open.
        
        if IsModifiedClick("CHATLINK") and MegaMacro_FrameText:HasFocus() then
            -- Note: In 12.0, getting spell info from a button is often simpler:
            local spellID = self:GetID() -- or self.spellID depending on frame type
            if not spellID or spellID == 0 then
                 -- fallback for slot-based buttons
                 -- local slot = SpellBook_GetSpellBookSlot(self); -- REMOVED in 12.0
            end

            -- If we can't reliably get the spellID from the button (due to new UI virtualization),
            -- we rely on the standard ChatEdit_InsertLink hook which we can't easily override here without tainting.
            
            -- Alternative: We don't hook the buttons, we hook ChatEdit_InsertLink
        end
    end
    
    -- 12.0 Hook Strategy: Hook the global Chat insertion logic.
    hooksecurefunc("ChatEdit_InsertLink", function(link)
        if MegaMacro_FrameText and MegaMacro_FrameText:HasFocus() then
            -- If our frame is focused, insert the link text there instead of the chat window
            -- Strip the link formatting to just get the name? Or keep the link?
            -- Macros usually want the name: [Spell Name] -> Spell Name
            local name = link:match("%[(.-)%]")
            if name then
                MegaMacro_FrameText:Insert(name)
                -- We might need to prevent it from pasting into the chat frame if possible, 
                -- but InsertLink is a notification hook.
            end
        end
    end)
end
...
function MecaMacroConfig_GenerateCheckbox(name, text, tooltip, x, y, checked, onClick)
    -- 12.0: Inherit BackdropTemplate for any frame that might need borders, even CheckButtons
    local checkbox = CreateFrame("CheckButton", "MegaMacro_ConfigContainer_" .. name, MegaMacro_ConfigContainer, "ChatConfigCheckButtonTemplate, BackdropTemplate")
...
function MecaMacroConfig_GenerateButton(name, text, tooltip, x, y, onClick)
    local button = CreateFrame("Button", "MegaMacro_ConfigContainer_" .. name, MegaMacro_ConfigContainer, "UIPanelButtonTemplate, BackdropTemplate")
